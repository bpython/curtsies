...
def peel_off_esc_code(s):
    ...
    m2 = re.match(
        "(?P<front>.*?)(?P<seq>(?P<csi>)(?P<command>[\x40-\x5f]))(?P<rest>.*)",
        s,
    ...
    if m1 and m2:
        m = (
            m1
            if len(m1.groupdict()["front"]) <= len(m2.groupdict()["front"])
            else m2
        )
...
def token_type(info):
    ...
    if info["command"] == "m":
        ...
        values = (
            cast(List[int], info["numbers"]) if len(info["numbers"]) else [0]
        )
