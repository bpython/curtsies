...
class FSArray(Sequence):
    ...
    def __setitem__(self, slicetuple, value):
        ...
        if slicesize(rowslice) != len(value):
            raise ValueError(
                "row dimensions do not match: %r, %r" % (len(value), rowslice)
            )
        self.rows = (
        ...
        for a_row, b_row in zip(a, b):
            ...
            for a_char, b_char in zip(a_row + extra_a, b_row + extra_b):
                if ignore_formatting:
                    a_char_for_eval = (
                        a_char.s if isinstance(a_char, FmtStr) else a_char
                    )
                    b_char_for_eval = (
                        b_char.s if isinstance(b_char, FmtStr) else b_char
                    )
                else:
...
if __name__ == "__main__":
    ...
    a[0:2, 5:11] = cast(
        Tuple[FmtStr, ...],
        (
            fmtstr("hey", "on_blue") + " " + fmtstr("yo", "on_red"),
            fmtstr("qwe qw"),
        ),
    )
    a.dumb_display()
