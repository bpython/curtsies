...
CURSES_NAMES[
    b"\x1b[F"
] = u"KEY_END"  # https://github.com/bpython/bpython/issues/490
CURSES_NAMES[
    b"\x1b[H"
] = u"KEY_HOME"  # https://github.com/bpython/bpython/issues/490
...
MAX_KEYPRESS_SIZE = max(
    len(seq)
    for seq in (list(CURSES_NAMES.keys()) + list(CURTSIES_NAMES.keys()))
)
...
def get_key(bytes_, encoding, keynames="curtsies", full=False):
    ...
    * bytes returns the original bytes from stdin (NOT unicode)
    ...
    if not all(isinstance(c, type(b"")) for c in bytes_):
        raise ValueError(
            "get key expects bytes, got %r" % bytes_
        )  # expects raw bytes
    if keynames not in ["curtsies", "curses", "bytes"]:
        raise ValueError(
            "keynames must be one of 'curtsies', 'curses' or 'bytes'"
        )
    seq = b"".join(bytes_)
    ...
    key_known = (
        seq in CURTSIES_NAMES or seq in CURSES_NAMES or decodable(seq, encoding)
    )
...
def could_be_unfinished_utf8(seq):
    ...
    if ord(seq[0:1]) & 0b10000000 == 0b10000000 and len(seq) < 1:
        return True
    elif ord(seq[0:1]) & 0b11100000 == 0b11000000 and len(seq) < 2:
        return True
    elif ord(seq[0:1]) & 0b11110000 == 0b11100000 and len(seq) < 3:
        return True
    elif ord(seq[0:1]) & 0b11111000 == 0b11110000 and len(seq) < 4:
        return True
    elif ord(seq[0:1]) & 0b11111100 == 0b11111000 and len(seq) < 5:
        return True
    elif ord(seq[0:1]) & 0b11111110 == 0b11111100 and len(seq) < 6:
        return True
    else:
        return False
