...
class Chunk(object):
    ...
    def repr_part(self):
        ...
        return (
            ...
            + (repr(self.s) if PY3 else repr(self.s)[1:])
            + ")" * len(atts_out)
        )
            ...
            if width + w > max_width:
                ...
                if width < max_width:
                    ...
                    return (
                        width + 1,
                        Chunk(
                            s[start_offset : self.internal_offset]
                            + replacement_char,
...
class FmtStr(object):
    ...
    def setitem(self, startindex, fs):
        ...
        return self.setslice_with_length(
            startindex, startindex + 1, fs, len(self)
        )
    ...
    def join(self, iterable):
        ...
        for i, s in enumerate(iterable):
            ...
            if isinstance(s, FmtStr):
                ...
                chunks.extend(
                    fmtstr(s).chunks
                )  # TODO just make a chunk directly
            else:
    ...
    def ljust(self, width, fillchar=None):
        ...
        if "bg" in shared:
            return (
                self + fmtstr(to_add, bg=shared[str("bg")]) if to_add else self
            )
        else:
            uniform = self.new_with_atts_removed("bg")
            return (
                uniform + fmtstr(to_add, **self.shared_atts)
                if to_add
                else uniform
            )
    ...
    def rjust(self, width, fillchar=None):
        ...
        if "bg" in shared:
            return (
                fmtstr(to_add, bg=shared[str("bg")]) + self if to_add else self
            )
        else:
            uniform = self.new_with_atts_removed("bg")
            return (
                fmtstr(to_add, **self.shared_atts) + uniform
                if to_add
                else uniform
            )
    ...
    for char, char_start, char_end in zip(s, divides[:-1], divides[1:]):
        if char_start == start and char_end == start:
            ...
            new_chunk_chars.extend(
                replacement_char
                * interval_overlap(char_start, char_end, start, end)
            )
...
def normalize_slice(length, index):
    ...
    if is_int:
        if index.start < 0 or index.start > length:
            raise IndexError(
                "index out of bounds: %r for length %s" % (index, length)
            )
    return index
