...
class TestFmtStrParsing(unittest.TestCase):
    ...
    def test_simple_escapes(self):
        self.assertEqual(
            str(fmtstr("\x1b[33mhello\x1b[0m")), "\x1b[33mhello\x1b[39m"
        )
        self.assertEqual(
            str(fmtstr("\x1b[33mhello\x1b[39m")), "\x1b[33mhello\x1b[39m"
        )
        self.assertEqual(str(fmtstr("\x1b[33mhello")), "\x1b[33mhello\x1b[39m")
        self.assertEqual(
            str(fmtstr("\x1b[43mhello\x1b[49m")), "\x1b[43mhello\x1b[49m"
        )
        self.assertEqual(
            str(fmtstr("\x1b[43mhello\x1b[0m")), "\x1b[43mhello\x1b[49m"
        )
        self.assertEqual(str(fmtstr("\x1b[43mhello")), "\x1b[43mhello\x1b[49m")
        self.assertEqual(
            str(fmtstr("\x1b[32;1mhello")),
            "\x1b[32m\x1b[1mhello\x1b[0m\x1b[39m",
        )
        self.assertEqual(str(fmtstr("\x1b[2mhello")), "\x1b[2mhello\x1b[0m")
        self.assertEqual(
            str(fmtstr("\x1b[32;2mhello")),
...
class TestFmtStrSplice(unittest.TestCase):
    ...
    def test_various_splices(self):
        ...
        self.assertEqual(
            c.splice("asdfg", 1, 5), blue("h") + "asdfg" + red("!")
        )
...
class TestFmtStr(unittest.TestCase):
    ...
    def test_split(self):
        self.assertEqual(
            blue("hello there").split(" "), [blue("hello"), blue("there")]
        )
        s = blue("hello there")
        ...
        self.assertEqual(
            blue("abcbd").split("b"), [blue("a"), blue("c"), blue("d")]
        )
...
class TestUnicode(unittest.TestCase):
    ...
    def test_len_of_unicode_in_fsarray(self):
        ...
        fsa.rows[0] = fsa.rows[0].setslice_with_length(
            0, 2, fmtstr("┌─", "blue"), 2
        )
        self.assertEqual(fsa.shape, (3, 2))
...
class TestCharacterWidth(unittest.TestCase):
    ...
    def test_width_aware_slice(self):
        self.assertEqual(
            fmtstr("Ｅ").width_aware_slice(slice(None, 1, None)).s, " "
        )
        self.assertEqual(
            fmtstr("Ｅ").width_aware_slice(slice(None, 2, None)).s, "Ｅ"
        )
        self.assertEqual(
...
class TestWidthHelpers(unittest.TestCase):
    def test_combining_char_aware_slice(self):
        ...
        self.assertEqual(
            width_aware_slice("ab\u0300\u0300c", 0, 2), "ab\u0300\u0300"
        )
